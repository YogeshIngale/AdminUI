...
export class QuestionSetComponent implements OnInit {
    ...
    submit(action: string) {
        ...
        if (action) {
            this.submitForm(this.formObj).subscribe(res => {
                ...
                this.formsDashBoardEvent.emit(false);
            }, (error) => {
                this.formsDashBoardEvent.emit(false);
            });
        } else {
    ...
    getTotalNonEmptyQuestionCount() {
        ...
        this.progressWidth =
            Math.floor((this.currentQuestionSetIndex / this.totalNonEmptyAssessmentData) * 100);
    }

    getPlaceholder(questionSet) {
        if ('placeholder' in questionSet) {
            let placeholder = questionSet.placeholder;
            return placeholder;
        } else {
            return "";
        }
